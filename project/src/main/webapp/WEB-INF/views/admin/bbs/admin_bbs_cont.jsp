			...
			<div class="col-md-12">	
				<h3>내용보기</h3>
					<c:if test="${empty rv.reply_cont}">
						<form method="post" action="admin_reply_ok" onsubmit="return reply_check();">
					</c:if>
					<c:if test="${!empty rv.reply_cont}">
						<form method="post" action="admin_reply_edit" onsubmit="return reply_check();">
					</c:if>
							<input type="hidden" value="${admin_name}" name="reply_name" />
				...
				<table class="table table-striped table-hover">
					...
					<tr>
						...
						<td colspan="4"><a href="./resources/upload1${b.bbs_file}">첨부파일</a></td>
					</tr>
					</c:if>
					<tr>
						<td colspan="6">
					...
					<c:if test="${!empty r_list}"> <!-- 댓글목록이 있는 경우 -->
						<c:forEach var="r" items="${r_list}">
							...
							<tr>
								<td colspan="2">${r.reply_date}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
									<a href="admin_bbs_cont?bbs_no=${b.bbs_no}&state=cont&page=${page}&reply_no=${r.reply_no}">[수정]</a>&nbsp;
									<a href="admin_reply_del?bbs_no=${b.bbs_no}&page=${page}&reply_no=${r.reply_no}">[삭제]</a>
								</td>
							</tr>						
						</c:forEach>
					</c:if>
        			...
        			<input class="btn btn-default btn-lg" type="button" value="목록"
        				onclick="location='admin_custom?page=${page}';" />     			
        			<input class="btn btn-default btn-lg" type="button" value="삭제"
        				onclick="location='admin_bbs_del_ok?page=${page}&bbs_no=${b.bbs_no}';" />
        				
        				
        				
        			<c:if test="${b.bbs_name=='관리자'}">
  					<input class="btn btn-default btn-lg" type="button" value="수정" 
  						onclick="location='admin_bbs_cont?bbs_no=${b.bbs_no}&page=${page}&state=edit';" /> 
  					</c:if> 		
